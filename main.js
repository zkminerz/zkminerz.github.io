...
function refreshData(){
    ...
    contractBalance(function(result){
        ...
        balanceElem.textContent = 'Contract Balance: ' + stripDecimals(rawStr, 3) + ' ETH';
    });
    
    web3.eth.getBalance(currentAddr).then(result => {
        rawStr = numberWithCommas(Number(web3.utils.fromWei(result)).toFixed(3));
        document.getElementById('userTrx').textContent = 'Your Balance: ' + stripDecimals(rawStr, 3) + ' ETH';
    }).catch((err) => {
        console.log(err)
    });
    ...
    getMyMiners(function(miners){
        ...
        calculateEggBuySimple(web3.utils.toWei('0.1'),function(eggs){
            devFee(eggs,function(fee){
                sellsforexampledoc.textContent='0.1 ETH Hires ' + formatEggs(eggs-fee) + ' miners';
            });
        });
    });
    updateBuyPrice()
...
function formatTrxValue(trxstr){
    return parseFloat(parseFloat(trxstr).toFixed(6));
}
